...
- **Instance Metrics Endpoints:** Added `/instance-metrics`, serving the latest `instance_stats` snapshot in Prometheus text format from its own registry, and `/instance-metrics/json`, serving the full history as a paginated JSON page. Exposes the instance counts per channel, version and architecture that Nebraska already records but never surfaced. ([#665](https://github.com/flatcar/nebraska/pull/665))
