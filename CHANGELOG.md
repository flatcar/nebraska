...
- **macOS development support:** The development scripts (`check_pkg_test.sh`, `setup_local_db.sh`, `run-all.sh`) no longer rely on `mapfile` (bash 4+) or GNU `getopt`, so they work with the bash 3.2 and BSD utilities shipped with macOS. The backend Makefile also drops `-extldflags "-static"` on macOS, where the system linker does not support static linking. A [macOS development guide](docs/development-macos.md) documents the setup. ([#374](https://github.com/flatcar/nebraska/issues/374))
