...
- **OEM/platform distribution per group:** A new `GET /api/apps/{appIDorProductID}/groups/{groupID}/oem_breakdown` endpoint reports how many instances in a group are running on each hardware or cloud platform, alongside a percentage, using the OEM value instances already report. Instances with no OEM are grouped as `unknown`. ([#1479](https://github.com/flatcar/nebraska/pull/1479))
