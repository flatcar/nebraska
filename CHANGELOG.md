...
- **Instances per OEM Metric:** Added `nebraska_instances_per_oem` Prometheus gauge exposing active instance counts by application and OEM platform (e.g. `aws`, `azure`, `vmware`, `qemu`), sourced from the `instance.oem` column populated via Omaha update requests.
