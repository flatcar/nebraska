...
- **helm: PostgreSQL persistence is enabled by default:** `postgresql.primary.persistence.enabled` now defaults to `true`, so a default install keeps the Nebraska database on a PersistentVolume instead of the container filesystem, where it was lost on every pod restart. Installations that deliberately want an ephemeral database must now set the value to `false`, and `helm install`/`helm upgrade` prints a warning when they do. Clusters without a default StorageClass need `postgresql.primary.persistence.storageClass` set, otherwise the PostgreSQL PersistentVolumeClaim stays pending.
