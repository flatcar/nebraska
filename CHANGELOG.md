...
- **Unsupported HTTP methods on API routes now return 405:** for example `GET /api/instances/{id}`, which is registered for `PUT` only, or `HEAD` on any route, since `backend/api/spec.yaml` declares no `head` operations. These were previously rejected as a 404 internally, which the frontend fallback then answered with 200 and `index.html`, so callers received an HTML page rather than an error. This changed with the `oapi-codegen/echo-middleware` 1.1.0 upgrade ([#1506](https://github.com/flatcar/nebraska/pull/1506)) and was not recorded at the time.
