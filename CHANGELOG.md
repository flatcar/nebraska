...
- **helm/postgresql: the default superuser password is now generated, not `changeIt`.** `postgresql.auth.postgresPassword` defaults to `""` and the chart generates a random 24-character password on first install, preserving any value already in the cluster across upgrades — restoring what the Bitnami subchart did before chart 2.0.0 overrode it with a fixed default. Retrieve it with `kubectl get secret <release>-postgresql -o jsonpath='{.data.postgres-password}' | base64 -d`. Anyone relying on the published default must set `postgresql.auth.postgresPassword` explicitly or use `postgresql.auth.existingSecret`.
...
- **helm: replaced the Bitnami PostgreSQL subchart with an in-chart StatefulSet on the official `postgres` image.** The chart no longer depends on `https://charts.bitnami.com/bitnami` and no longer ships a `bitnamilegacy/*` image, so the bundled database receives security patches again. This is a **breaking change for installs with `postgresql.primary.persistence.enabled: true`**: the Bitnami data directory cannot be reused (different path, uid, and C library), so a `pg_dump`/restore is required — see the chart README "Upgrading to 3.0.0". Installs using an external database (`postgresql.enabled: false`) or the default ephemeral database are unaffected. Chart version bumped to 3.0.0. ([#1574](https://github.com/flatcar/nebraska/issues/1574), [#1148](https://github.com/flatcar/nebraska/issues/1148))
...
- **helm/postgresql: the `password` key is removed from the `<release>-postgresql` Secret.** The Bitnami subchart emitted both `postgres-password` (superuser) and `password` (a separate app user); this chart has a single superuser and emits only `postgres-password`. Helm deletes the absent key on upgrade, so anything reading it — backup jobs, external tooling — must be repointed **before** upgrading.
