...
func TestOIDCEndpointBehavior(t *testing.T) {
	t.Run("config_route_public", func(t *testing.T) {
		// establish db connection
		db := newDBForTest(t)
		...
		// Test /config endpoint - not registered in OIDC mode
		req, err := http.NewRequest("GET", fmt.Sprintf("%s/config", testServerURL), nil)
		require.NoError(t, err)
