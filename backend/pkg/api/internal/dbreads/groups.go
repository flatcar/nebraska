...
func (q *Queries) GetGroupVersionCountTimeline(groupID string, duration string) (map[time.Time](types.VersionCountMap), bool, error) {
	...
	queryWg.Go(func() error {
		...
		rows, err := q.db.Queryx(instancesWithoutStatusQuery, groupID)
		if err != nil {
			return err
		}
		defer rows.Close()
	...
	queryWg.Go(func() error {
		...
		statusHistoryRows, err := q.db.Queryx(instanceWithStatusHistoryInInterval, groupID)
		if err != nil {
			return err
		}
	...
	queryWg.Go(func() error {
		...
		versionAggRows, err := q.db.Queryx(instancesWithoutStatusInIntervalQuery, groupID)
		if err != nil {
			return err
		}
