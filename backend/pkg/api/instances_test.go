...
func TestRegisterInstance(t *testing.T) {
	...
	_, err := a.RegisterInstance(Instance{ID: "", IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup.ID, "1.0.0"))
	assert.Error(t, err, "Using empty string as instance id.")

	_, err = a.RegisterInstance(Instance{ID: instanceID, IP: "invalidIP"}, NewInstanceApplication(tApp.ID, tGroup.ID, "1.0.0"))
	assert.Error(t, err, "Using an invalid instance ip.")

	_, err = a.RegisterInstance(Instance{ID: instanceID, IP: "10.0.0.1"}, NewInstanceApplication("invalidAppID", tGroup.ID, "1.0.0"))
	assert.Error(t, err, "Using an invalid application id.")

	_, err = a.RegisterInstance(Instance{ID: instanceID, IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, "invalidGroupID", "1.0.0"))
	assert.Error(t, err, "Using an invalid group id.")

	_, err = a.RegisterInstance(Instance{ID: instanceID, IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, "invalidGroupID", ""))
	assert.Error(t, err, "Using an empty instance version.")

	_, err = a.RegisterInstance(Instance{ID: instanceID, IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, "invalidGroupID", "aaa1.0.0"))
	assert.Equal(t, ErrInvalidSemver, err, "Using an invalid instance version.")

	_, err = a.RegisterInstance(Instance{ID: instanceID, IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup2.ID, "1.0.0"))
	assert.Equal(t, ErrInvalidApplicationOrGroup, err, "The group provided doesn't belong to the application provided.")

	instance, err := a.RegisterInstance(Instance{ID: instanceID, Alias: "myalias", IP: "10.0.0.1", OEM: "azure", AlephVersion: "2.9.1.1-r1"}, NewInstanceApplication("{"+tApp.ID+"}", "{"+tGroup.ID+"}", "1.0.0"))
	...
	instance, err = a.RegisterInstance(Instance{ID: instanceID, Alias: "mynewalias", IP: "10.0.0.2"}, NewInstanceApplication(tApp.ID, tGroup.ID, "1.0.2"))
	...
	_, err = a.RegisterInstance(Instance{ID: instanceID, IP: "10.0.0.2"}, NewInstanceApplication(tApp2.ID, tGroup.ID, "1.0.2"))
	assert.Error(t, err, "Application id cannot be updated.")

	instance, err = a.RegisterInstance(Instance{ID: instanceID, IP: "10.0.0.3", OEM: "gcp", AlephVersion: "3.0.0"}, NewInstanceApplication(tApp.ID, tGroup3.ID, "1.0.3"))
...
func TestGetInstancesSearch(t *testing.T) {
	...
	_, _ = a.RegisterInstance(Instance{ID: uuid.New().String(), Alias: instanceAlias, IP: "10.0.0.4"}, NewInstanceApplication(tApp.ID, tGroup.ID, "1.0.4"))
...
func TestGetInstancesFiltered(t *testing.T) {
	...
	for idx, id := range []string{instanceID1, instanceID2, instanceID3, instanceID4} {
		...
		_, _ = a.RegisterInstance(Instance{ID: id, IP: ip}, NewInstanceApplication(tApp.ID, tGroup.ID, "1.0.0"))
	}
...
func TestUpdateInstanceStats(t *testing.T) {
	...
	_, err = a.GetUpdatePackage(Instance{ID: tInstance1.ID, IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup.ID, "1.0.0"))
	assert.NoError(t, err)

	_, err = a.GetUpdatePackage(Instance{ID: tInstance2.ID, IP: "10.0.0.2"}, NewInstanceApplication(tApp.ID, tGroup.ID, "1.0.1"))
	assert.NoError(t, err)
	...
	_, err = a.GetUpdatePackage(Instance{ID: tInstance1.ID, IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup.ID, "1.0.3"))
	assert.NoError(t, err)

	_, err = a.GetUpdatePackage(Instance{ID: tInstance2.ID, IP: "10.0.0.2"}, NewInstanceApplication(tApp.ID, tGroup.ID, "1.0.4"))
	assert.NoError(t, err)

	_, _ = a.RegisterInstance(Instance{ID: uuid.New().String(), IP: "10.0.0.4"}, NewInstanceApplication(tApp.ID, tGroup.ID, "1.0.5"))
