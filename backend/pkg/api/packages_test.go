...
func TestAddPackage(t *testing.T) {
	...
	pkg, err := a.AddPackage(&Package{Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg"), Version: "12.1.0", ApplicationID: tApp.ID, ChannelsBlacklist: []string{tChannel1.ID, tChannel2.ID}, Arch: ArchAArch64})
	assert.NoError(t, err)

	_, err = a.AddPackage(&Package{Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg"), Version: "12.1.0", ApplicationID: tApp.ID, Arch: ArchX86})
	assert.NoError(t, err)

	pkgX, err := a.GetPackage(pkg.ID)
	assert.NoError(t, err)
	assert.Equal(t, PkgTypeOther, pkgX.Type)
	assert.Equal(t, "http://sample.url/pkg", pkgX.URL.String)
	...
	_, err = a.AddPackage(&Package{URL: null.StringFrom("http://sample.url/pkg"), Version: "12.1.0", ApplicationID: tApp.ID})
	...
	_, err = a.AddPackage(&Package{Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg"), Version: "aaa12.1.0"})
	assert.Equal(t, ErrInvalidSemver, err, "Package version must be a valid semver.")

	_, err = a.AddPackage(&Package{Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg"), Version: "12.1.0"})
	assert.Error(t, err, "App id is required and must be a valid uuid.")

	_, err = a.AddPackage(&Package{Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg"), Version: "12.1.0", ApplicationID: tApp.ID, ChannelsBlacklist: []string{uuid.New().String()}})
	assert.Error(t, err, "Blacklisted channels must be existing channels ids.")

	_, err = a.AddPackage(&Package{Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg"), Version: "12.1.0", ApplicationID: tApp.ID, ChannelsBlacklist: []string{"invalidChannelID"}})
	...
	_, err = a.AddPackage(&Package{Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg"),
	...
	_, err = a.AddPackage(&Package{Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg"), Version: "12.3.0", ApplicationID: tApp.ID, Arch: Arch(77777)})
...
func TestAddPackageFlatcar(t *testing.T) {
	...
	pkg := &Package{
		Type:          PkgTypeFlatcar,
		URL:           null.StringFrom("https://update.release.flatcar-linux.net/amd64-usr/XYZ/"),
...
func TestUpdatePackage(t *testing.T) {
	...
	tPkg, err := a.AddPackage(&Package{Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg"), Version: "12.1.0", ApplicationID: tApp.ID, ChannelsBlacklist: []string{tChannel1.ID}})
	assert.NoError(t, err)
	...
	err = a.UpdatePackage(&Package{ID: tPkg.ID, Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg_updated"), Version: "12.2.0", ChannelsBlacklist: []string{tChannel2.ID, tChannel3.ID}})
	assert.NoError(t, err)

	pkg, err := a.GetPackage(tPkg.ID)
	assert.NoError(t, err)
	assert.Equal(t, "http://sample.url/pkg_updated", pkg.URL.String)
	...
	err = a.UpdatePackage(&Package{ID: tPkg.ID, Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg_updated"), Version: "12.2.0", ChannelsBlacklist: []string{tChannel4.ID}})
	assert.Equal(t, ErrBlacklistingChannel, err)

	err = a.UpdatePackage(&Package{ID: tPkg.ID, Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg_updated"), Version: "12.2.0", ChannelsBlacklist: nil, Arch: ArchAArch64})
	assert.NoError(t, err)
...
func TestDeletePackage(t *testing.T) {
	...
	tTeam, _ := a.AddTeam(&Team{Name: "test_team"})
	tApp, _ := a.AddApp(&Application{Name: "test_app", TeamID: tTeam.ID})
	tPkg, err := a.AddPackage(&Package{Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg"), Version: "12.1.0", ApplicationID: tApp.ID})
	assert.NoError(t, err)
...
func TestGetPackage(t *testing.T) {
	...
	tPkg, err := a.AddPackage(&Package{Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg"), Version: "12.1.0", ApplicationID: tApp.ID, ChannelsBlacklist: []string{tChannel.ID}})
	assert.NoError(t, err)

	pkg, err := a.GetPackage(tPkg.ID)
	assert.NoError(t, err)
	assert.Equal(t, PkgTypeOther, pkg.Type)
	assert.Equal(t, "http://sample.url/pkg", pkg.URL.String)
...
func TestGetPackageByVersionAndArch(t *testing.T) {
	...
	tPkg, err := a.AddPackage(&Package{Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg"), Version: "12.1.0", ApplicationID: tApp.ID})
	assert.NoError(t, err)
	tPkgARM, err := a.AddPackage(&Package{Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg"), Version: "13.2.1", ApplicationID: tApp.ID, Arch: ArchAArch64})
	assert.NoError(t, err)
...
func TestGetPackages(t *testing.T) {
	...
	tTeam, _ := a.AddTeam(&Team{Name: "test_team"})
	tApp, _ := a.AddApp(&Application{Name: "test_app", TeamID: tTeam.ID})
	_, _ = a.AddPackage(&Package{Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg1"), Version: "1010.5.0+2016-05-27-1832", ApplicationID: tApp.ID, Arch: ArchAMD64})
	_, _ = a.AddPackage(&Package{Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg2"), Version: "12.1.0", ApplicationID: tApp.ID, Arch: ArchX86})
	_, _ = a.AddPackage(&Package{Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg3"), Version: "14.1.0", ApplicationID: tApp.ID, Arch: ArchAArch64})
	_, _ = a.AddPackage(&Package{Type: PkgTypeOther, URL: null.StringFrom("http://sample.url/pkg4"), Version: "1010.6.0-blabla", ApplicationID: tApp.ID})

	pkgs, err := a.GetPackages(tApp.ID, 0, 0)
	assert.NoError(t, err)
	assert.Equal(t, 4, len(pkgs))
	assert.Equal(t, "http://sample.url/pkg4", pkgs[0].URL.String)
	assert.Equal(t, "http://sample.url/pkg1", pkgs[1].URL.String)
	assert.Equal(t, "http://sample.url/pkg3", pkgs[2].URL.String)
	assert.Equal(t, "http://sample.url/pkg2", pkgs[3].URL.String)
...
func TestMetadataPackages(t *testing.T) {
	...
	_, _ = a.AddPackage(
		&Package{
			Type:            PkgTypeOther,
			URL:             null.StringFrom("http://sample.url/pkg1"),
