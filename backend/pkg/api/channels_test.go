...
func TestAddChannel(t *testing.T) {
	...
	channel, err := as.AddChannel(&Channel{Name: "channel1", Color: "blue", ApplicationID: tApp.ID, PackageID: null.StringFrom(tPkg.ID)})
	...
	channel2, err := as.AddChannel(&Channel{Name: "channel2", Color: "green", ApplicationID: tApp.ID})
	...
	_, err = as.AddChannel(&Channel{Name: "channel3"})
	assert.Error(t, err, "App id is required")

	_, err = as.AddChannel(&Channel{Name: "channel3", ApplicationID: "invalidAppID"})
	assert.Error(t, err, "App id must be a valid uuid.")

	_, err = as.AddChannel(&Channel{Name: "channel3", ApplicationID: uuid.New().String()})
	assert.Error(t, err, "App used must exist.")

	_, err = as.AddChannel(&Channel{Name: "channel3", ApplicationID: tApp.ID, PackageID: null.StringFrom("invalidPackageID")})
	assert.Error(t, err, "Package id must be a valid uuid.")

	_, err = as.AddChannel(&Channel{Name: "channel3", ApplicationID: tApp.ID, PackageID: null.StringFrom(uuid.New().String())})
	assert.Error(t, err, "Package used must exist.")

	_, err = as.AddChannel(&Channel{Name: "channel3", ApplicationID: tApp.ID, PackageID: null.StringFrom(tPkg2.ID)})
	assert.Equal(t, ErrInvalidPackage, err, "Package used must belong to the same application as the channel.")

	_, err = as.AddChannel(&Channel{Name: "channel3", ApplicationID: tApp.ID, Arch: Arch(77777)})
	assert.Equal(t, ErrInvalidArch, err, "Channel must have a valid arch")

	_, err = as.AddChannel(&Channel{Name: "channel3", ApplicationID: tApp.ID, PackageID: null.StringFrom(tPkg.ID), Arch: ArchAArch64})
...
func TestUpdateChannel(t *testing.T) {
	...
	tPkg2, _ := as.AddPackage(&Package{Type: PkgTypeOther, URL: "http://sample.url/pkg", Version: "12.1.1", ApplicationID: tApp.ID})
	tPkg3, _ := as.AddPackage(&Package{Type: PkgTypeOther, URL: "http://sample.url/pkg", Version: "12.1.2", ApplicationID: tApp2.ID})
	tPkg4, _ := as.AddPackage(&Package{Type: PkgTypeOther, URL: "http://sample.url/pkg", Version: "12.1.3", ApplicationID: tApp.ID, ChannelsBlacklist: []string{tChannel.ID}})

	err := as.UpdateChannel(&Channel{ID: tChannel.ID, Name: "test_channel_updated", PackageID: null.StringFrom(tPkg2.ID)})
	assert.NoError(t, err)
	...
	err = as.UpdateChannel(&Channel{ID: tChannel.ID, Name: "test_channel_updated", PackageID: null.StringFrom(tPkg3.ID)})
	assert.Equal(t, ErrInvalidPackage, err, "Package used must belong to the same application as the channel.")

	err = as.UpdateChannel(&Channel{ID: tChannel.ID, PackageID: null.StringFrom(tPkg4.ID)})
...
func TestGetChannel(t *testing.T) {
	...
	tTeam, _ := as.AddTeam(&Team{Name: "test_team"})
	tApp, _ := as.AddApp(&Application{Name: "test_app", TeamID: tTeam.ID})
	tChannel, err := as.AddChannel(&Channel{Name: "test_channel", Color: "blue", ApplicationID: tApp.ID, Arch: ArchAArch64})
	assert.NoError(t, err)
...
func TestGetChannels(t *testing.T) {
	...
	tTeam, _ := as.AddTeam(&Team{Name: "test_team"})
	tApp, _ := as.AddApp(&Application{Name: "test_app", TeamID: tTeam.ID})
	_, err := as.AddChannel(&Channel{Name: "test_channel1", Color: "blue", ApplicationID: tApp.ID})
	assert.NoError(t, err)

	_, err = as.AddChannel(&Channel{Name: "test_channel2", Color: "green", ApplicationID: tApp.ID})
	assert.NoError(t, err)

	_, err = as.AddChannel(&Channel{Name: "test_channel3", Color: "red", ApplicationID: tApp.ID})
	assert.NoError(t, err)
