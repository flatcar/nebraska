...
func TestGetGroupOEMBreakdown(t *testing.T) {
	...
	tTeam, err := as.AddTeam(&Team{Name: "oem_bd_team"})
	assert.NoError(t, err)
	tApp, err := as.AddApp(&Application{Name: "oem_bd_app", TeamID: tTeam.ID})
	assert.NoError(t, err)
	tPkg, err := as.AddPackage(&Package{Type: PkgTypeOther, URL: "http://sample.url/pkg", Version: "12.1.0", ApplicationID: tApp.ID})
	assert.NoError(t, err)
	tChannel, err := as.AddChannel(&Channel{Name: "oem_bd_channel", Color: "blue", ApplicationID: tApp.ID, PackageID: null.StringFrom(tPkg.ID)})
	assert.NoError(t, err)
	tGroup, err := as.AddGroup(&Group{Name: "oem_bd_group", ApplicationID: tApp.ID, ChannelID: null.StringFrom(tChannel.ID), PolicyUpdatesEnabled: true, PolicySafeMode: true, PolicyPeriodInterval: "15 minutes", PolicyMaxUpdatesPerPeriod: 2, PolicyUpdateTimeout: "60 minutes"})
	assert.NoError(t, err)

	_, err = rs.RegisterInstance(Instance{ID: uuid.New().String(), IP: "10.0.0.1", OEM: "azure"}, runtime.NewInstanceApplication(tApp.ID, tGroup.ID, "1.0.0"))
	assert.NoError(t, err)
	_, err = rs.RegisterInstance(Instance{ID: uuid.New().String(), IP: "10.0.0.2", OEM: "azure"}, runtime.NewInstanceApplication(tApp.ID, tGroup.ID, "1.0.0"))
	assert.NoError(t, err)
	_, err = rs.RegisterInstance(Instance{ID: uuid.New().String(), IP: "10.0.0.3", OEM: "aws"}, runtime.NewInstanceApplication(tApp.ID, tGroup.ID, "1.0.0"))
	assert.NoError(t, err)
	_, err = rs.RegisterInstance(Instance{ID: uuid.New().String(), IP: "10.0.0.4"}, runtime.NewInstanceApplication(tApp.ID, tGroup.ID, "1.0.0"))
	assert.NoError(t, err)
