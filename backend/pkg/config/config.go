...
func Parse() (*Config, error) {
	...
	f.String("oidc-client-id", "", "OIDC client ID used for authentication")
	f.String("oidc-client-secret", "", fmt.Sprintf("OIDC client Secret used for authentication; can be taken from %s env var too", oidcClientIDEnvName))
	f.String("oidc-issuer-url", "", fmt.Sprintf("OIDC issuer URL used for authentication;can be taken from %s env var too", oidcClientSecretEnvName))
	...
	// Load from env
	k := koanf.New(".")
	if err := k.Load(env.Provider("NEBRASKA_", "_", func(s string) string {
		if s == "NEBRASKA_URL" {
			return strings.ToLower(s)
		}
		return strings.Replace(strings.ToLower(
			strings.TrimPrefix(s, "NEBRASKA_")), "_", "-", -1)
	}), nil); err != nil {
