...
test.describe('OIDC Authorization - Admin User', () => {
  ...
  test('admin should have access to instances data', async ({ request }) => {
    ...
    // Find the application that has groups (our test data)
    const testApp = appsResult.data?.applications?.find(app => 
      app.groups && app.groups.length > 0
    );
    
    expect(testApp).toBeDefined();
    expect(testApp.groups.length).toBeGreaterThan(0);
    
    // Use any group from the application - we just want to test admin access
    const anyGroup = testApp.groups[0];
    
    // admin should have access to instances endpoint (not be forbidden)
    const instancesResult = await oidcHelpers.makeAuthenticatedRequest(
      request, 'GET', `${baseUrl}/api/apps/${testApp.id}/groups/${anyGroup.id}/instances`, adminToken.token
    );
    
    // Admin should not be forbidden (403) or unauthorized (401)
    // The endpoint might return 400 for various validation reasons, but not auth issues
